...
function DiffViewer({ file, scrollTarget }) {
  ...
  const [flashedRowIndex, setFlashedRowIndex] =
    useState(null);
  ...
  useEffect(() => {
    ...
    if (
      !scrollTarget ||
      scrollTarget.path !== file.path
    ) {
      return;
    }

    const selector =
      `[data-row-index="${scrollTarget.rowIndex}"]`;
