...
function GlobalSearch({ comparison, onNavigate }) {
  ...
  const isOpen = !dismissed && trimmed.length >= 2 && Boolean(comparison);
  ...
  const results = useMemo(() => {
    ...
    for (const file of comparison.files) {
      ...
      for (let index = 0; index < file.changes.length; index++) {
        const row = file.changes[index];

        const leftHit = row.left && row.left.toLowerCase().includes(needle);

        const rightHit = row.right && row.right.toLowerCase().includes(needle);
    ...
    <div className="global-search" ref={containerRef}>
      ...
      {isOpen && (
        <div className="global-search-results">
          {results.length === 0 ? (
            <div className="no-files">No matches found.</div>
          ) : (
            <>
              <div className="global-search-summary">
                {totalMatches} match
                {totalMatches === 1 ? "" : "es"} in {results.length} file
                {results.length === 1 ? "" : "s"}
              </div>

              {results.map((group) => (
                <div key={group.file.path} className="search-file-group">
                  <div className="search-file-group-header">
                    <span className={`status ${group.file.status}`}>
                      {getStatusLabel(group.file.status)}
                    </span>

                    <span className="search-file-path">{group.file.path}</span>
                  </div>

                  {group.matches.map((match) => (
                    <button
                      key={`${match.rowIndex}-${match.side}`}
                      type="button"
                      className="search-match-row"
                      onClick={() => handleSelect(group.file, match.rowIndex)}
                    >
                  ...
                  {group.overflow > 0 && (
                    <div className="search-match-overflow">
                      ...
                      {group.overflow === 1 ? "" : "es"} in this file
                    </div>
                  )}
                </div>
              ))}
...
function highlightMatch(text, query) {
  ...
  while (matchIndex !== -1) {
    if (matchIndex > cursor) {
      parts.push(<span key={cursor}>{text.slice(cursor, matchIndex)}</span>);
    }
